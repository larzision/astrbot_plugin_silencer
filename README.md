...
3. **高峰期静默**：DeepSeek峰谷定价那段时间自动闭嘴，省小钱钱
...
| wake指令 | 发 "wake" | 当前会话 | 管理员 或 wake_whitelist |
...
- `peak_hours_enabled` — 开启后自动在 DeepSeek 峰谷时段闭嘴省 token
  - 每天 **9:00** 自动睡 180 分钟到 **12:00**
  - 每天 **14:00** 自动睡 240 分钟到 **18:00**
  - 强制全局睡眠，无视 `sleep_scope` 设置；高峰期结束后自动恢复原来的 scope
  - 插件启动时如果刚好在高峰期内，会立刻进入睡眠并补足剩余时长
